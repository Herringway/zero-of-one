 ...
 *    able to work.
 */
int add_punctuation_nodes(ZoO_knowledge* k) {
	int error;
	char[2] w;
	ZoO_index i, id;
 ...
 */
void finalize_word(ZoO_knowledge_word* w) {
	if (w.word != null) {
		free(w.word);
 ...
 *    {k.sorted_indices}.
 */
int ZoO_knowledge_find(const ZoO_knowledge* k, const ZoO_char* word, ZoO_index* result) {
	ZoO_index r;

	if (ZoO_sorted_list_index_of(k.words_count, k.sorted_indices, word, ZoO_index.sizeof, &cmp_word, k, &r) == 0) {
 ...
 */
int ZoO_knowledge_learn(ZoO_knowledge* k, const ZoO_char* word, ZoO_index* result) {
	...
	if (*result < k.words_count) {
		...
		memmove(k.sorted_indices + *result + 1, k.sorted_indices + *result, ((k.words_count - *result) * ZoO_index.sizeof));
	}
